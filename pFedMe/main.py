...
def main(dataset, algorithm, model, batch_size, learning_rate, beta, lamda, num_glob_iters,
    ...
    for i in range(times):
        ...
        global_cfmtx_record = server.train()
        # server.test()
    if not Path(f"results/{algorithm}").exists():
        os.makedirs(f"results/{algorithm}")
    json.dump(global_cfmtx_record, open(f"results/{algorithm}/global_cfmtx_record.json", "w"), cls=NumpyEncoder)
    # Average data 
    # if(algorithm == "PerAvg"):
    #     algorithm == "PerAvg_p"
    # if(algorithm == "pFedMe"):
